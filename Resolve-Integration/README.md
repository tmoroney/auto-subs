...
- **Template**: `src-tauri/resources/AutoSubs.lua` contains `[[__AUTOSUBS_RESOURCES_FOLDER__]]` / `[[__AUTOSUBS_APP_EXECUTABLE__]]` / `[[__AUTOSUBS_MAILBOX_DIR__]]` placeholders. The mailbox one substitutes to a `[[...]]` path or `nil` (Linux package, where the shared script can't carry a per-user path and bootstrap derives it from the environment).
