...
- **Template**: `src-tauri/resources/AutoSubs.lua` contains `[[__AUTOSUBS_RESOURCES_FOLDER__]]` / `[[__AUTOSUBS_APP_EXECUTABLE__]]` placeholders, plus `__AUTOSUBS_MAILBOX_DIR__` — a bare expression the app substitutes with a `[[...]]` path on Windows (ANSI-safe so `loadfile` sees the mailbox). Unsubstituted it reads as a nil global and bootstrap derives the path from the environment, so installer templates don't need to handle it.
