...
def transcribe_and_print_progress():
    ...
    model = stable_whisper.load_faster_whisper('distil-small.en', device=device, compute_type="int8")

    # Start the transcription process and capture the progress
    result = model.transcribe_stable('C:\\Users\\thoma\\Documents\\AutoSubs\\auto-subs-audio.wav', language="en", vad_filter=True)
