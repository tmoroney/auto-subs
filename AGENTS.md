...
* **Opening the app from Resolve**: a `mode = "manual"` boot (Utility/Dev script) also launches the desktop app via `bmd.openfileexternal("Open", app_executable)` in bootstrap.lua — the only process-launch primitive left in the sandbox. The app is single-instance, so this just focuses the window when it's already running. `mode = "startup"` must never launch it (Resolve opening shouldn't pop AutoSubs up).
