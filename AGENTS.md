...
* **Script install owned by the app**: release builds write `Utility/AutoSubs.lua` into the user's Resolve Scripts folder at startup ([resolve_scripts.rs](AutoSubs-App/src-tauri/src/resolve_scripts.rs)) from bundled `[[...]]`-placeholder templates, regenerating only when content changes — so updater installs refresh it without a reinstall — and delete the `AutoSubs.scriptlib` that 3.10.0 installed. Installers also drop the launcher at install time: NSIS ExecWaits `AutoSubs.exe --install-resolve-scripts` (reuses the same Rust path), the macOS pkg postinstall sed-substitutes the templates, and deb/rpm ship copies pre-substituted by `scripts/gen-resolve-linux.js`. Dev builds skip it (`npm run setup-resolve` owns the dev launcher).
* **Manual start, resident afterwards**: the user runs Workspace > Scripts > AutoSubs once per Resolve session; the loop then stays up after the app closes and the app never stops it. **Never run a long-lived loop via `fusion:Execute`** (3.10.0 did, from a startup scriptlib): that state shares Fusion's script executor with macro control scripts (`BTNCS_Execute`, `INPS_ExecuteOnChange`), `comp:Execute` and the Console, and `bmd.wait` doesn't yield it, so every scripted macro control went dead and Fusion text fields lost focus for the whole session. The Scripts-menu state doesn't have this problem. See [resident-bridge-fusion-regression.md](Resolve-Integration/docs/resident-bridge-fusion-regression.md).
* **The Resolve script cannot open the app** (free 21.1, verified 2026-09-19): `bmd.openfileexternal` is gone from `bmd` (what's left: file/dir exists, paths, uuid, time, `wait`, `UIDispatcher`, `scriptapp`, `readstring`/`writestring`, which serialise tables and are not file I/O), `os` only has `clock/date/difftime/getenv/time/tmpname`, `fusion:Execute("!Py3: …")` returns ok but runs nothing, and UI Manager windows (and so clickable links) are a Studio feature that triggers the upgrade nag. Don't retry these routes; launching the app would need a watcher outside Resolve.
* **One loop at a time**: launchers check liveness via `bridge_alive()` — a fresh legacy `Heartbeat` (old loops) or a `Probe`/`ProbeAck` token echo (new loops; the idle loop writes no prefs on purpose — every bound `fusion:` call is marshaled through Resolve's UI event queue, and a queued prefs event landing during shutdown teardown crashed Resolve in `FusionApp::PrefsChanged`). The winner claims `Owner` with a unique token (last-writer-wins after a settle window); a running loop that sees a foreign `Owner` exits. The Utility/Dev scripts launch with `mode = "manual"` (takeover: set `Stop` to now, wait for the old loop to clear it, start fresh) — this also shuts down a loop a 3.10.0 scriptlib left running. A loop exits when `Stop` differs from the value it read at startup, so a takeover aimed at a busy predecessor still lands and never kills the replacement. Each mailbox request is claimed via `Global.AutoSubsBridge.Claim`, so two briefly-coexisting loops can't handle the same request twice.
