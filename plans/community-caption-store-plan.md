# Community caption style store — updated plan (2026-09-05)

Supersedes `plans/preset-gallery.md` (written 2026-06-13). The core approach there is still right (GitHub-repo-backed, no server, PR/issue moderation, jsDelivr CDN, reuse `importPreset`). What follows is what has drifted and what's missing.

## 1. What the old plan got wrong / what changed
...
### P0 — prerequisites in the Resolve layer (needed for thumbnails + safety)
1. **`GeneratePreview` must honour `presetSettings`**: call the macro's `SetInputValues(presetSettings)` after loading the template, before render. Also add optional `frame` (default midpoint) and a fixed output size (Resize node → 640×360) so gallery images are uniform.
2. **`SetInputValues` ignores unknown keys**: check `key` against `InputKeys` (or `pcall`) so a preset from a newer macro degrades gracefully.
3. **Expose a compat version**: return `caption_template_version` from a new `GetMacroVersion` endpoint (or include it in the existing template-list response). Presets carry `minTemplateVersion` (date string, lexically comparable).

### P1 — capture previews for *local* presets (closes an existing gap, prerequisite for Share)
- In `create-preset-flow.tsx`, after `capturePresetSettings()` succeeds, call `generatePreview({presetSettings, exportPath: appData/caption-previews/<id>.png})` and set `previewImage`. `PresetThumbnail` then just works. Also render one for each built-in on first run (or ship static PNGs under `src/assets`).
...
1. Folder in `auto-subs` (recommended) vs. dedicated repo?
2. Pin CDN to `@main` (live) or to the app's release tag (stable, but new presets wait for a release)? Recommend `@main` since presets are data.
3. Issue-form → bot-PR submission (recommended) vs. asking contributors to PR directly.
4. Should built-in presets also live in `community-presets/` so they're rendered/validated the same way?
